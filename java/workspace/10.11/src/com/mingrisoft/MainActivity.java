...
public class MainActivity extends Activity {
	...
	private boolean isPlayKeySound=true;
	@Override
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		if(item.getGroupId()==R.id.setting){			//判断是否选择了参数设置菜单组
			if(item.getItemId()==R.id.bgsound){
			...
			}else if(item.getItemId()==R.id.keysound){
				if(item.isChecked()){					//当菜单项已经被选中
					item.setChecked(false);			//设置菜单项不被选中
