...
import net.sunniwell.tools.AESTools;
...
public class PppoePage extends SWFragment implements OnClickListener {
	...
	private final String PRE_PAGE = PppoePage.class.getPackage().getName() + ".EthernetPage";
	...
	private void configPppoe() {
		try {
			...
			if (!SWUtil.isEmpty(name) && !SWUtil.isEmpty(pass)) {
				...
				// name = AESTools.Encrypt(name);
				// pass = AESTools.Encrypt(pass);
				mPppoeManager.setConfig(name, pass, PppoeManager.DEVICE_TYPE_ETHERNET);
